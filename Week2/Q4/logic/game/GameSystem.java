package HW.Week2.Q4.logic.game;

import java.util.ArrayList;

import HW.Week2.Q4.logic.unit.*;
...
public class GameSystem {
	...
	public boolean isGameEnd() {
		if(allRedPieces.size()<=2 || allWhitePieces.size()<=2) setGameEnd(true);
		return gameEnd;
	}
	...
	public void printBoardStatus(ArrayList<BaseUnit> allPieces) {
		for(int i = 0;i<allPieces.size();i++) {
			...
			System.out.println(temp.getClass().getSimpleName() + " " + temp.getName()+ ": ("+ temp.getColumn()+", "+ temp.getRow()+")" + " hp: " + temp.getHp());
		}
	...
	public BaseUnit promote(BaseUnit baseUnit, int choice) {
		int col = baseUnit.getColumn();
		int row = baseUnit.getRow();
		boolean iW = baseUnit.isWhite();
		String name = baseUnit.getName();
		
		BaseUnit BU;
		if(choice == 0) {
			BU = new MeleeUnit(col, row, iW, name);
		}
		else if(choice == 1) {
			BU = new RangeUnit(col, row, iW, name);
		}
		else if(choice == 2) {
			BU = new FlyingUnit(col, row, iW, name);
		}
		else {return null;}
		return BU;
	}
